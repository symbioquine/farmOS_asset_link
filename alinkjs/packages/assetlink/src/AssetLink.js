...
export default class AssetLink {
  ...
  async boot() {
    ...
    this._memory.on('update', transform => {
      console.log('_memory::update', transform);
      ...
      operations.forEach(operation => {
        ...
        if (['updateRecord', 'replaceAttribute', 'addToRelatedRecords', 'removeFromRelatedRecords', 'replaceRelatedRecords', 'replaceRelatedRecord'].includes(operation.op) && operation.record.type.startsWith('asset--')) {
          this.eventBus.$emit('changed:asset', { type: operation.record.type, id: operation.record.id});
        }

        if (['addRecord', 'updateRecord'].includes(update.operations.op) && operation.record.type.startsWith('log--')) {
          operation.record.relationships.asset.data.forEach(assetRel => {
            this.eventBus.$emit('changed:assetLogs', { type: assetRel.type, id: assetRel.id});
          });
